...
import carsData from "../assets/carsData"; // ✅ FIXED IMPORT
...
const HomeCars = () => {
  ...
  const fetchCars = async () => {
    ...
    try {
      ...
      console.log("HomeCars /api/cars response:", res.data);
      const json = res.data;
      setCars(Array.isArray(json) ? json : json.cars || json.data || []);
    } catch (err) {
      const isCanceled =
  ...
  }; //Get Image if not present then a placegolder there
  ...
  }; //get from when till when it is booked and when it will be availabe
