...
const carSchema = new Schema({
  ...
  bookings: { type: [carBookingSubSchema], default: [] }, // Shows the bookings for this car
});
...
carSchema.methods.isAvailableForRange = function (
  ...
  if (!requestPickup || !requestReturn) return false;

  const start = new Date(requestPickup);
  const end = new Date(requestReturn);
