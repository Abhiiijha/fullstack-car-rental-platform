...
// Edit Modal
const EditModal = ({ car, categories, onClose, onSubmit, onChange }) => {
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {inputField("Year", "year", "number", { required: true })}
              {inputField("Category", "category", "select", {
                required: true,
                items: (categories || []).filter((c) => c !== "all"),
              })}

              {inputField("Status", "status", "text", { required: true })}
              {inputField("Daily Rate", "dailyRate", "number", {
                required: true,
              })}
              {inputField("Mileage", "mileage", "number", { required: true })}
              {inputField("Transmission", "transmission", "text", {
                required: true,
              })}
              {inputField("Fuel Type", "fuelType", "text", { required: true })}
              {inputField("Seats", "seats", "number", { required: true })}
            </div>

            {inputField("Image URL", "image", "text", { required: true })}
  ...
  //MAP INPUT FIELD TO EDIT AND SEND UPDATED VERSION TO BACKEND
};

const NoCarView = ({ onResetFilter }) => (
  <div className={`${styles.gradientGray} ${styles.noCarsContainer}`}>
    ...
    <h3 className="mt-4 text-xl font-medium text-white">NO Car found</h3>
...
const ManageCar = () => {
  ...
  const fetchCars = useCallback(async () => {
    try {
      const res = await api.get("/api/cars", { params: { limit: 100 } });
      const json = res.data;
      const raw = json.cars || json.data || [];

      setCars(
        raw.map((c, i) => {
          const baseCar = buildSafeCar(c, i);
          return {
            ...baseCar,
            image: c.image ? makeImageUrl(c.image) : baseCar.image,
  ...
  return (
    <div className="min-h-screen bg-gray-950 p-4 sm:p-6">
      ...
      <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl p-5 mb-6 border border-b-gray-800">
      ...
      {filterCars.length === 0 && (
        <NoCarView onResetFilter={() => setCategoryFilter("all")} />
      )}

      {showEditModal && editingCar && (
        <EditModal
          car={editingCar}
          categories={categories} // fixed: pass categories
          onClose={() => {
            setShowEditModal(false);
            setEditingCar(null);
          }}
